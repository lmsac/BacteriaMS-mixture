...
function.names = local({
  env = globalenv()
  names = ls(env)
  names[sapply(names, function(x) 'function' %in% class(get(envir = env, x = x)))]
})
...
search.datebase.mixture.jackknife = function(sample.peaklist, reference.spectra, tolerance, mix.number = 2, method = 'cosine', jackknife.ratio = 1/3, jackknife.number = 100) {
  ...
  foreach(peaklist = peaklists, .combine = 'rbind', 
          .export = c(#'reference.spectra', 'tolerance', 'mix.number', 'method',
                      function.names)) %dopar% 
