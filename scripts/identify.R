...
results = lapply(list.dirs(full.names = F, recursive = F), function(x) {
  lapply(list.dirs(x, full.names = F, recursive = F), function(y) {
    ...
    result = search.datebase.mixture(combined.peaklist[, 1:2], reference.spectra, tolerance, mix.number = mix.number)[['cosine']][1, ]
    
    setwd('../..')
    message(paste0(Sys.time(), ': ', x, '/', y))
    result
  })
})
